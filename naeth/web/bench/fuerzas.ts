...
async function main() {
  ...
  let pasos = 0
  while (pasos < 600 && sim.paso()) pasos++
  ...
  const nodos = sim.nodos.map((n) => ({ id: n.id, x: n.x, y: n.y, vx: 0, vy: 0 }))
  const porId = new Map(nodos.map((n) => [n.id, n]))
  const aristas = model.edges
    .filter((e) => porId.has(e.source) && porId.has(e.target))
    .map((e) => ({ source: e.source, target: e.target }))

  const link = forceLink(aristas as never[]).id((d: never) => (d as { id: string }).id).distance(34).strength(0.6)
  const carga = forceManyBody().strength(-38).distanceMax(600)
  const s = forceSimulation(nodos as never[]).force('link', link).force('charge', carga).stop()

  salida.textContent = ''
  linea(`CORPUS   ${model.nodes.length} nodos · ${aristas.length} aristas · ${pasos} pasos hasta asentarse`)
  ...
  const distancia = mide(60, () => {
    ...
    ;(link as unknown as { initialize: (n: unknown[], r: () => number) => void })
      .initialize(nodos as never[], Math.random)
  })
  linea('  cambiar la distancia de arista (viva)     ' + f3(distancia.med) + f3(distancia.p90) + f3(distancia.max))
  ...
  const anade = Math.max(distancia.p90, repulsion.p90)
  ...
  linea(`  Reconstruir cuesta ${(recon.med / Math.max(distancia.med, 0.001)).toFixed(0)}x mas que reconfigurar.`)
  linea()
