...
import { buildTree, hidingFolder } from './tree'
...
describe('hidingFolder · senalar no puede abrir lo que cerraste', () => {
  ...
  it('si no hay nada colapsado, no esconde nadie', () => {
    expect(hidingFolder('naeth/core', new Set())).toBe(null)
  })

  it('el subtema colapsado la esconde', () => {
    expect(hidingFolder('naeth/core', new Set(['o:naeth/core']))).toBe('o:naeth/core')
  })
  ...
  it('el proyecto colapsado gana al subtema: es el grupo mas externo', () => {
    ...
    expect(hidingFolder('naeth/core', c)).toBe('p:naeth')
  })

  it('un colapso de OTRA rama no la esconde', () => {
    expect(hidingFolder('naeth/core', new Set(['p:cenit', 'o:naeth/viewer']))).toBe(null)
  })
  ...
  it('una nota sin path cae en el mismo sitio que la pone `buildTree`', () => {
    ...
    // fallan en silencio.
    expect(hidingFolder(null, new Set(['p:(sin path)']))).toBe('p:(sin path)')
    expect(hidingFolder('', new Set(['o:(sin path)/·']))).toBe('o:(sin path)/·')
  })

  it('un path de un solo nivel usa el subtema por defecto', () => {
    expect(hidingFolder('naeth', new Set(['o:naeth/·']))).toBe('o:naeth/·')
  })
})
